let postingFormCats = document.getElementById('visibleElement2');
let saveButtonCats = document.getElementById('saveButton');

async function sentCatsPosting(frontName, selectedCategoryId) {
...
async function getCatBreeds() {
    let response = await fetch("/api/posting/getcatbreeds", {
        method: 'GET',
        headers: {
...
async function getCatsForm(frontName, selectedCategoryId) {
    let CatsBreeds = await getCatBreeds();
    let dropmenu = '<select class="custom-select custom-select-sm" required>\n';
    for (var i = 0; i < CatsBreeds.length; i++) {
        dropmenu += '<option id="breed" name="breed" value="' + CatsBreeds[i].breed + '">' + CatsBreeds[i].breed + '</option>\n';
    }
    dropmenu += '</select>\n';
    dropmenu += '</div>';
    saveButtonCats.onclick = () => sentCatsPosting(frontName, selectedCategoryId);
    postingFormCats.innerHTML = '<div id="parameters" class="main-container">\n' +
